...
def runLivePlotInThread(livePlotWidget, thread):
    livePlotWidget.show()
    thread.started.connect(livePlotWidget.show)
    thread.start()
...
class liveGraphWidget(QWidget):
    def __init__(self):
        super().__init__()
        layout = QHBoxLayout()
        self.label = QLabel("Live Plot of EMG")
    ...
    def setRecordingStatus(self, recordingStatus):
        self.recording = recordingStatus
    
    def setEMGPlot(self, EMGPlot):
...
class MyWidget(QWidget):
    def __init__(self):
        ...
        # Setting up separate thread for plotting
        self.live_plot_widget = liveGraphWidget()
        """
        self.thread = QThread()
        self.plottingData = self.dataProcessing()
        self.plottingData.moveToThread(self.thread)
        self.thread.started.connect(self.plottingData.run)
        self.plottingData.finished.connect(self.thread.quit)
        self.plottingData.finished.connect(self.worker.deleteLater)
        self.plottingData.finished.connect(self.thread.deleteLater)
        """
...
if __name__ == "__main__":
    ...
    live_plot_thread = QThread()
    widget = MyWidget()
    widget.live_plot_widget.moveToThread(live_plot_thread)
    runLivePlotInThread(widget.live_plot_widget, live_plot_thread)
