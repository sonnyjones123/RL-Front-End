...
class TDLambda():
    def __init__(self, studyID = None, numInputs = 3, alpha = 0.01, gammas = [0.9, 0.71, 0.75], lambda_ = 0.9):
        """
    ...
    def update(self, newState, Znew):
        ...
        if self.loopCount == 0:
            ...
            self.eTrace = self.lambda_ * np.minimum(self.eTrace + self.featureVector, 1)

            # Updating weights
            self.weights += self.alpha * tdError.T * self.eTrace
